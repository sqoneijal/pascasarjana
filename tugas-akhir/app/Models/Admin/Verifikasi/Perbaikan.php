...
class Perbaikan extends Common
{
   ...
   {
      $table = $this->db->table('tb_status_tugas_akhir tsta');
      $table->select('tsta.id, tsta.nim, tsta.nama, tsta.angkatan, concat(tp.jenjang, \' \', tp.nama) as program_studi, tsta.status, tsta.id_periode, tst.short_name as status_tesis');
      $table->join('tb_prodi tp', 'tp.id_feeder = tsta.id_prodi');
