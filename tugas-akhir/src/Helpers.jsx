...
export const getFile = (filename, folder = "") => {
   ...
   return `${protocol}//${hostname}:8012/media${folderPath}/${filename}`;
};
...
export const detail_label = (label, value, span = 0) => {
   return (
      <Row className="mb-2">
         <Col md={span} sm={12} className={`fw-semibold text-muted`}>
            {label}
         </Col>
      ...
      <tr className="odd">
         <td colSpan={colSpan} className="dataTables_empty" style={{ verticalAlign: "top" }}>
            <div className="d-flex flex-column flex-center">
               <div className="fs-4 fw-bolder text-dark mb-4">Tidak ada data yang ditemukan.</div>
            </div>
         </td>
      </tr>
   );
...
export const keterangan_halaman = (title, content = []) => {
   return (
            ...
            <div className="d-flex flex-column">
               {(() => {
                  ...
                  if (content.length > 1) {
                     content.forEach((row) => {
                        list.push(
                           <li className="d-flex align-items-center" key={row}>
                              <span className="bullet bullet-dot bg-primary me-5" /> {row}
                           </li>
                        );
                        return list;
                     });
...
export const keteranganLampiran = (key) => {
   const object = {
      buku_kegiatan_akademik: "Buku Kegiatan Akademik (Pengecekan Kehadiran pada Seminar Sinopsis yang Berlaku)",
      permohonan_proposal: "Permohonan Kepada Direktur Pascasarjana UIN Ar-Raniry, Melalui Kasubbag TU",
      persetujuan_penasehat: "Persetujuan Penasehat Akademik / Ka.Prodi",
      review_jurnal: "Review Jurnal sebanyak 5 Review Jurnal",
      sinopsis_disertasi: "Sinopsis Disertasi sesuai Ketentuan Yang Berlaku sebanyak 6 Eksamplar",
      slip_seminar: "Slip Setoran Seminar Proposal",
      slip_spp: "Slip Setoran SPP yang Sedang Berjalan",
      transkrip_nilai: "Transkrip Nilai",
   };

   return parse(key, object) ? parse(key, object) : "";
};

export const form_check = (label, name, config = {}, marginTop = 0) => {
   const uniqueID = `${name}_${uid(32)}`;

   return (
      <div className={`form-check mt-${marginTop}`}>
         <input className="form-check-input" type="checkbox" name={name} id={uniqueID} {...config} />
         {label && (
            <label className="form-check-label text-gray-800" htmlFor={uniqueID}>
               {label}
            </label>
         )}
      </div>
...
export const form_check_switch = (label, name, config = {}, variant = "solid", size = 0) => {
   const uniqueID = `${name}_${uid(32)}`;

   return (
      <div className={`form-check form-switch form-check-custom form-check-${variant}`} style={{ display: "inline" }}>
         <input className={`form-check-input ${size || ""}`} type="checkbox" name={name} id={uniqueID} {...config} />
         {label && (
...
export const form_radio = (label, name, config = {}, marginTop = 0) => {
   const uniqueID = `${name}_${uid(32)}`;

   return (
      <div className={`form-check mt-${marginTop}`}>
         <input className="form-check-input" type="radio" name={name} id={uniqueID} {...config} />
         {label && (
            <label className="form-check-label text-gray-800" htmlFor={uniqueID}>
               {label}
            </label>
...
export const getPeriodeAktif = (data) => {
   let maxPeriode = -Infinity;
   let statusT = null;
   ...
   for (const item of data) {
      ...
      // Ambil nilai terbesar dari periode jika status tidak ditemukan
...
export const renderStatusTesis = (key, lists) => {
   const find = lists.find((e) => parse("id", e) === key);
   return `<span class="text-danger">${parse("short_name", find)}</span>`;
};
