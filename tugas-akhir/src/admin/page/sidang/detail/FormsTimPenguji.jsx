...
const FormsTimPenguji = () => {
   const { module, init } = useSelector((e) => e.redux);
   const { openFormTimPenguji, pageType, detailContent, daftarKategoriKegiatan, detailPenguji } = module;
   ...
   useLayoutEffect(() => {
      if (openFormTimPenguji && pageType === "update" && h.objLength(detailPenguji)) {
         setInput({ ...detailPenguji });
         setSelectedKategorikegiatan([{ id: h.parse("id_kategori_kegiatan", detailPenguji), label: h.parse("kategori_kegiatan", detailPenguji) }]);
      }
      return () => {};
   }, [openFormTimPenguji, detailPenguji, pageType]);
   ...
   const handleClose = () => {
      dispatch(setModule({ ...module, openFormTimPenguji: false, pageType: "", detailPenguji: {} }));
      clearProps();
   };
   ...
   const submit = (e) => {
      ...
      fetch.then((res) => {
         ...
         dispatch(setModule({ ...module, openFormTimPenguji: false, pageType: "", detailPenguji: {}, tim_penguji_sidang: data.content }));
         clearProps();
         h.dtReload();
      });
      fetch.finally(() => {
   ...
   const props = { input, errors, setInput };

   return (
      <React.Fragment>
         {openFormTimPenguji && <div className="drawer-overlay" style={{ zIndex: 999 }} />}
         <div className={`bg-white drawer drawer-end min-w-25 ${openFormTimPenguji ? "drawer-on" : ""}`} style={{ zIndex: 999 }}>
            <Card className="rounded-0 w-100">
               <Card.Header className="pe-5">
                  <div className="card-title">
                     <div className="d-flex justify-content-center flex-column me-3">
                        <span className="fs-4 fw-bold text-gray-900 text-hover-primary me-1 lh-1">{h.pageType(pageType)} Dosen Penguji</span>
                     </div>
                  </div>
               ...
               <Card.Body className="hover-scroll-overlay-y">
                  ...
                  {h.parse("apakah_dosen_uin", input) === "f" ? <FormsBukanDosenUIN {...props} /> : <FormsDosenUIN {...props} />}
                  <Typeahead
...
export default FormsTimPenguji;
