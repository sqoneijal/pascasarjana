...
import FormsPerbaikiSidangMunaqasyah from "./FormsPerbaikiSidangMunaqasyah";
...
const Context = () => {
   ...
   const tabMenus = [
      ...
      { value: 4, label: "Jadwal Sidang" },
      { value: 5, label: "Tim Penguji" },
   ];

   const sudahMelaksanakanSidangMunaqasyah = (e) => {
      e.preventDefault();
      const confirm = h.confirm("Apakah benar mahasiswa ini sudah melaksanakan sidang munaqasyah?");
      confirm.then((res) => {
         ...
         setIsSubmit(true);
         const fetch = h.post(`/submitsudahmelaksanakansidang`, formData);
         fetch.then((res) => {
            if (typeof res === "undefined") return;
            ...
            dispatch(setModule({ ...module, tim_penguji: data.content, detailContent: { ...detailContent, status: h.parse("status_tesis", data) } }));
            h.dtReload();
         });
         fetch.finally(() => {
            setIsSubmit(false);
            ...
            <Card className="rounded-0 w-100">
               <Card.Header className="pe-5">
                  <div className="card-title">
                     <div className="d-flex justify-content-center flex-column me-3">
                        <span className="fs-4 fw-bold text-gray-900 text-hover-primary me-1 lh-1">Detail Sebagai Penguji Sidang Munaqasyah</span>
                     </div>
                  </div>
               ...
               <Card.Body className="hover-scroll-overlay-y">
                  {isLoading ? (
                     ...
                     <React.Fragment>
                        <FormsPerbaikiSidangMunaqasyah />
                        <Identitas />
                        <div className="mb-5 hover-scroll-x mt-5 fw-bold">
                                 ...
                                 <Each
                                    ...
                                    render={(row) => (
                                       <li className="nav-item">
                                          <a
                                             className={`nav-link btn btn-color-gray-600 rounded-bottom-0 ${
               ...
               {[26, 27, 28].includes(h.parse("status", detailContent)) && (
                  <Card.Footer className="text-end">
                     <ButtonGroup>
                        ...
                        {h.buttons(`Sudah Melaksanakan Sidang Munaqasyah`, isSubmit, {
                           onClick: isSubmit ? null : sudahMelaksanakanSidangMunaqasyah,
                        })}
                     </ButtonGroup>
                  </Card.Footer>
               )}
