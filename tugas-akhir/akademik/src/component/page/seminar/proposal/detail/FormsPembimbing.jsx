...
const FormsPembimbing = () => {
   ...
   const { openFormsPembimbing, pageType, penelitian, daftarKategoriKegiatan, detailContent, detailPembimbing } = module;
   ...
   useLayoutEffect(() => {
      if (pageType === "update" && h.objLength(detailPembimbing)) {
         setInput({
            ...detailPembimbing,
            apakah_dosen_uin: h.parse("apakah_dosen_uin", detailPembimbing) === "t",
            dosen: h.parse("nidn", detailPembimbing),
         });
         setSelectedDosen([{ id: h.parse("nidn", detailPembimbing), label: h.parse("nama_dosen", detailPembimbing) }]);
   ...
   const handleClose = () => {
      clearProps();
      dispatch(setModule({ ...module, openFormsPembimbing: false, detailPembimbing: {} }));
   };
   ...
   const submit = (e) => {
      ...
      fetch.then((res) => {
         ...
         dispatch(
            setModule({
               ...module,
               openFormsPembimbing: false,
               pageType: "insert",
               ...data.content,
               detailPembimbing: {},
               detailContent: { ...detailContent, status: data.content.status },
            })
         );
         h.dtReload();
      });
   ...
   return (
      <React.Fragment>
         ...
         <div
            className={`bg-white drawer drawer-end ${openFormsPembimbing ? "drawer-on" : ""}`}
                  ...
                  <Row>
                     <Col md={3} sm={12}>
                        {h.form_text(
                           ...
                           {
                              onChange: (e) => setInput((prev) => ({ ...prev, [e.target.name]: e.target.value })),
                              value: h.parse(`pembimbing_ke`, input),
                           },
                           true,
                           errors
                        )}
