...
const FormsPembimbing = () => {
   ...
   const { openFormsPembimbing, pageType, detailContent, daftarKategoriKegiatan } = module;
   ...
   const handleClose = () => {
      clearProps();
      dispatch(setModule({ ...module, openFormsPembimbing: false, pageType: "" }));
   };

   const submit = (e) => {
      e.preventDefault();
      const formData = { pageType, user_modified: h.parse("username", init), id_status_tugas_akhir: h.parse("id", detailContent) };
      Object.keys(input).forEach((key) => (formData[key] = input[key]));

      setIsSubmit(true);
      const fetch = h.post(`/submittimpembimbing`, formData);
      fetch.then((res) => {
         ...
         clearProps();
         dispatch(setModule({ ...module, openFormsPembimbing: false, pageType: "", pembimbing: data.content }));
         h.dtReload();
      });
      fetch.finally(() => {
         setIsSubmit(false);
